...
def parse_ivankra():
    ...
    try:
        ...
        for row in reader:
            # Required columns — skip row if missing
            simplified_raw = row.get('Simplified', '').strip()
            pinyin_raw = row.get('Pinyin', '').strip()
            level = row.get('Level', '').strip()

            if not simplified_raw or not pinyin_raw or not level:
                continue

            # Handle variants like "爸爸|爸" — use first variant
            simplified = simplified_raw.split('|')[0].strip()
        ...
        raise RuntimeError(f"CSV parse error in ivankra data: {e}") from e
...
def parse_drkameleon():
    ...
    if not isinstance(drka_data, list):
        raise RuntimeError(f"Unexpected data format from drkameleon: expected list, got {type(drka_data).__name__}")
        ...
        if isinstance(forms, list) and forms:
            ...
            if isinstance(first_form, dict):
                ...
                if isinstance(meanings, list) and meanings:
                    english_lookup[simp] = '; '.join(str(m) for m in meanings[:3])
